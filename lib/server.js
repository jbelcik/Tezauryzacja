...
var onUpdatePoints = function(data) {
    var updatePlayerPoints = playerById(this.id), i, you = -1;
    ...
    for (i = 0; i < players.length; i += 1) {
        ...
        if (this.id.slice(0, 8) == players[i].id.slice(0, 8)) {
            you = i;
        }
...
var onSendItem = function(data) {
    socket.sockets.socket(data.to).emit("item sent", {item: data.item});
};
